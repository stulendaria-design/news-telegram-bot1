...
SENT_FILE = "/tmp/sent.json"          # Все ссылки, которые когда-либо отправлялись
APPROVED_FILE = "/tmp/approved.json"  # Ссылки, которые были одобрены и опубликованы
# =====================
...
def load_approved():
    if os.path.exists(APPROVED_FILE):
        with open(APPROVED_FILE, 'r') as f:
            return json.load(f)
    return []  # список словарей {link, title, date}

def save_approved(link, title):
    approved = load_approved()
    approved.append({'link': link, 'title': title, 'date': str(datetime.now(MSK))})
    with open(APPROVED_FILE, 'w') as f:
        json.dump(approved[-100:], f)  # храним последние 100

def parse_time(time_str):
    """Парсит время из строки типа 'Wed, 13 May 2026 10:30:00 GMT'"""
    try:
...
async def cmd_last(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Принудительная проверка новостей"""
    await update.message.reply_text("🔍 Проверяю RSS, ждите...")
    await check_rss_now(context.application)
    await update.message.reply_text("✅ Готово! Если были новые новости — они уже у вас в чате.")

async def cmd_digest_now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отправляет дайджест одобренных новостей за неделю"""
    approved = load_approved()
    ...
    print(f"📊 Отправлен дайджест: {len(weekly)} новостей")

async def check_rss_now(application):
    """Немедленная проверка RSS"""
...
async def scheduled_check(application):
    """Запланированная проверка (работает постоянно)"""
    print(f"[{datetime.now(MSK)}] Планировщик запущен")
    while True:
        now = datetime.now(MSK)
        # Проверяем каждые 30 минут, но только с 8:00 до 23:00
        if 8 <= now.hour < 23:
            # Ждём до ближайших 00 или 30 минут
            if now.minute < 30:
...
async def weekly_digest_job(application):
    """Раз в неделю отправляет дайджест в канал"""
    while True:
        now = datetime.now(MSK)
        # В следующее воскресенье в 10:00
